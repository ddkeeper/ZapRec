...
export default function PipWindow() {
  ...
  useEffect(() => {
    const unlistenSize = window.caplet.onPipSizeChanged((newSize) => {
      console.log('[PipWindow] Size changed:', newSize)
      setSize(newSize)
    })
    const unlistenShape = window.caplet.onPipShapeChanged((newShape) => {
  ...
  const toggleShape = () => {
    ...
    setShape(newShape)
    window.caplet.setPipShape(newShape)
    window.caplet.setPipSize(size)
  }

  const toggleSize = () => {
    const nextSize = size === 'sm' ? 'md' : size === 'md' ? 'lg' : 'sm'
    setSize(nextSize)
    window.caplet.setPipSize(nextSize)
  }
