import { DesktopSource, CameraSettings } from '../types'
...
export class MediaCapturer {
  ...
  async getDisplaySources(): Promise<DesktopSource[]> {
    const sources = await window.screenApi.getSources(['screen', 'window']) as DesktopSource[]
    return sources.map(s => ({ ...s, display_id: s.id }))
  }
...
async startDisplayCapture(
    ...
    const maxWidth = _width || 1920
    const maxHeight = _height || 1080
  ...
  async startWindowCapture(windowId: string, _width?: number, _height?: number, _fps?: number): Promise<MediaStream> {
    this.stopDisplayCapture()
